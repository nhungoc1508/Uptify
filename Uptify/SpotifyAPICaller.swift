...
//  Uptify
//
//  Created by Ngoc Hoang on 30/12/2021.
//
...
class SpotifyAPICaller: BDBOAuth1SessionManager {
    static let client = SpotifyAPICaller(baseURL: URL(string: "https://accounts.spotify.com/authorize?"), consumerKey: "fe9df09de79a44f7ac4a27aeeb46db3e", consumerSecret: "7fe34b48e9a84aeea382d8471d76f0e5")
    var loginSuccess: (() -> ())?
    ...
    func login(url: String, success: @escaping () -> (), failure: @escaping (Error) -> ()) {
        ...
        SpotifyAPICaller.client?.fetchRequestToken(withPath: url, method: "GET", callbackURL: URL(string: "alamoSpotify://oauth"), scope: nil, success: { (requestToken: BDBOAuth1Credential!) -> Void in
            let url = URL(string: "https://accounts.spotify.com/authorize?response_type=code&client_id=fe9df09de79a44f7ac4a27aeeb46db3e&scope=user-read-private%20user-read-email&redirect_uri=http%3A%2F%2Flocalhost%3A8888%2Fcallback%2F&state=mkfbK6EZRQrvelf6")!
...
//    }
//
//    func getCurrentUser(success: @escaping (NSDictionary) -> (), failure: @escaping (Error) -> ()){
//        let url = "https://api.twitter.com/1.1/account/verify_credentials.json"
//        TwitterAPICaller.client?.get(url, parameters: ["include_email": true], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
//            success(response as! NSDictionary)
//        }, failure: { (task: URLSessionDataTask?, error: Error) in
//            failure(error)
//        })
